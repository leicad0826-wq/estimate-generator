...
def parse_betto(text):
    ...
    for r in range(100, 160):
        for c in range(14, 20):
            if ws.cell(row=r, column=c).value == '最終単価':
                tanka = ws.cell(row=r, column=17).value
                break
        if tanka:
            break
    betto_text = None
    for r in range(100, 160):
        for c in range(18, 24):
            if ws.cell(row=r, column=c).value == '別途請求内容':
                betto_text = ws.cell(row=r+1, column=c).value
                break
        if betto_text is not None:
            break
    ...
    anchor = etree.Element(f'{{{NS_XDR}}}twoCellAnchor')
    ...
    cNvPr.set('id', str(img_id))
    cNvPr.set('name', img_name)
    cNvPicPr = etree.SubElement(nv, f'{{{NS_XDR}}}cNvPicPr')
    locks = etree.SubElement(cNvPicPr, f'{{{NS_A}}}picLocks')
...
def build_drawing(tmpl_drawing_bytes, tmpl_rels_bytes, images_list, all_files, media_prefix):
    ...
    for idx, (img_data,) in enumerate(images_list):
        ...
        col_from = idx * col_width
        col_to = col_from + col_width - 1
    ...
    ws['AD4'] = tomorrow.year
    ws['AH4'] = tomorrow.month
    ws['AK4'] = tomorrow.day
    ws['F11'] = d['nouki']
    ...
    for idx, ds in enumerate(datasets):
        ...
        if idx == 0:
            ...
            ws = wb_out.copy_worksheet(wb_out.worksheets[0])
            ws.title = sheet_name
        fill_sheet(ws, ds['d'])
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="/xl/drawings/drawing{idx+1}.xml" Id="rId1"/>
            ...
            if dns is None:
                dns = etree.SubElement(wb_xml, f'{{{NS_S}}}definedNames')
